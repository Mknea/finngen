...
class Person:
    ...
    def __repr__(self) -> str:
        ...
        shown_fields = self.__dict__
        for field_name in list(shown_fields.keys()):
            if field_name[0] == "_":
                shown_fields[field_name[1:]] = getattr(self, field_name)
                del shown_fields[field_name]
        return (
            "Person(" + ", ".join(f"{key}={value!r}" for key, value in shown_fields.items()) + ")"
        )
    ...
    def birthday(self):
        ...
        with equal weight on each day of the birth year.
